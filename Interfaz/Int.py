...
def generar_mensaje_retroalimentacion(datos):
    ...
    if datos['clima'] in ["Lluvioso", "Húmedo"]:
        mensajes.append("Los días húmedos pueden intensificar la rigidez. Considera mantenerte abrigado y hacer estiramientos suaves.")
    if datos['dieta'] == "Inflamatoria":
        mensajes.append("Notamos una dieta pro-inflamatoria. Incorporar más vegetales o pescado puede hacer una gran diferencia.")
    ...
    if datos['sueno'] in ["Malo", "Horrible"]:
        mensajes.append(f"Un sueño de calidad '{datos['sueno'].lower()}' afecta la fatiga. Priorizar el descanso es clave.")
    elif datos['sueno'] in ["Excelente", "Bueno"]:
        mensajes.append("¡Qué bien que hayas tenido una buena noche de sueño! Es fundamental.")
    if datos['actividad'] == "Sí":
        mensajes.append("¡Felicidades por mantenerte activo! Cada movimiento cuenta.")
    else:
        if dolor < 3 and fatiga < 3:
            mensajes.append("En días con menos síntomas, una caminata suave puede ayudar a la movilidad.")
        else:
            mensajes.append("Escuchar a tu cuerpo y descansar en días difíciles es muy importante.")
    return "\n\n- ".join(mensajes)
...
def mostrar_registro_diario(panel_contenedor):
    ...
    rigidez_var, dolor_var, inflamacion_var, fatiga_var = (tk.StringVar(value="0") for _ in range(4))
    estado_animo_var, actividad_fisica_var, calidad_sueno_var, tipo_dieta_var, clima_actual_var = (tk.StringVar(value="-- Seleccionar --") for _ in range(5))
    ...
    tk.Label(formulario, text=" Clima y Entorno", font=("Arial", 12, "bold"), bg="white", fg="#4B0082").grid(row=fila_actual, column=0, columnspan=3, sticky="w", pady=(20, 5))
    fila_actual += 1
    tk.Label(formulario, text="Tipo de Clima", bg="white").grid(row=fila_actual, column=0, sticky="w")
    ttk.Combobox(formulario, textvariable=clima_actual_var, values=["Soleado", "Seco", "Ventoso", "Nublado", "Lluvioso", "Húmedo"], state="readonly", width=20).grid(row=fila_actual + 1, column=0, sticky="w", padx=10)
    tk.Label(formulario, text="Temperatura (°C)", bg="white").grid(row=fila_actual, column=1, sticky="w", padx=40)
    tk.Entry(formulario, textvariable=temperatura_var, width=15).grid(row=fila_actual + 1, column=1, sticky="w", padx=40)
    tk.Label(formulario, text="Ej: 28.5", font=("Arial", 8), fg="gray", bg="white").grid(row=fila_actual + 2, column=1, sticky="w", padx=40)
    fila_actual += 3
    ...
    ttk.Combobox(formulario, textvariable=estado_animo_var, values=["Feliz", "Tranquilo", "Optimista", "Neutral", "Estresado", "Ansioso", "Irritable", "Frustrado", "Triste"], state="readonly", width=20).grid(row=fila_actual + 1, column=0, sticky="w", padx=10)
    fila_actual += 3
    ...
    ttk.Combobox(formulario, textvariable=actividad_fisica_var, values=["Sí", "No"], state="readonly", width=20).grid(row=fila_actual + 1, column=0, sticky="w", padx=10)
    tk.Label(formulario, text="Calidad del Sueño", bg="white").grid(row=fila_actual, column=1, sticky="w", padx=40)
    ttk.Combobox(formulario, textvariable=calidad_sueno_var, values=["Excelente", "Bueno", "Ok", "Malo", "Horrible"], state="readonly", width=20).grid(row=fila_actual + 1, column=1, sticky="w", padx=40)
    fila_actual += 3

    tk.Label(formulario, text=" Nutrición y Dieta", font=("Arial", 12, "bold"), bg="white", fg="#4B0082").grid(row=fila_actual, column=0, columnspan=3, sticky="w", pady=(20, 5))
    fila_actual += 1
    tk.Label(formulario, text="Tipo de Dieta", bg="white").grid(row=fila_actual, column=0, sticky="w")
    ttk.Combobox(formulario, textvariable=tipo_dieta_var, values=["Antiinflamatoria", "Balanceada", "Inflamatoria"], state="readonly", width=20).grid(row=fila_actual + 1, column=0, sticky="w", padx=10)
    fila_actual += 3

    def validar_y_guardar_registro():
        campos_obligatorios = [v.get() for v in [rigidez_var, dolor_var, inflamacion_var, fatiga_var, estado_animo_var, actividad_fisica_var, calidad_sueno_var, tipo_dieta_var, clima_actual_var]]
        ...
        datos_para_feedback = {k: v.get() for k, v in zip(['dolor', 'fatiga', 'clima', 'dieta', 'sueno', 'actividad'], [dolor_var, fatiga_var, clima_actual_var, tipo_dieta_var, calidad_sueno_var, actividad_fisica_var])}
        nueva_fila = [datetime.datetime.now().strftime("%Y-%m-%d"), clima_actual_var.get(), temperatura, 1 if actividad_fisica_var.get() == "Sí" else 0, calidad_sueno_var.get(), tipo_dieta_var.get(), rigidez_var.get(), dolor_var.get(), inflamacion_var.get(), fatiga_var.get(), estado_animo_var.get()]
        nombre_archivo = f"{usuario_actual}_registros.csv"
        try:
            file_exists = os.path.exists(nombre_archivo)
            with open(nombre_archivo, 'a', newline='', encoding='utf-8') as file:
                writer = csv.writer(file)
                if not file_exists:
                    writer.writerow(['fecha', 'clima', 'temperatura_C', 'actividad_fisica', 'sueño', 'dieta_tipo', 'rigidez_score', 'dolor_score', 'inflamacion_score', 'fatiga_score', 'estado_animo'])
                writer.writerow(nueva_fila)
            messagebox.showinfo("Análisis Diario", generar_mensaje_retroalimentacion(datos_para_feedback), parent=panel_contenedor)
        except Exception as e:
            messagebox.showerror("Error de Guardado", f"Ocurrió un error al guardar: {e}", parent=panel_contenedor)

    tk.Button(scrollable_frame, text=" Guardar y Analizar Registro", bg="#4B0082", fg="white", font=("Arial", 12, "bold"), width=30, command=validar_y_guardar_registro).pack(pady=40)
...
def abrir_ventana_principal():
    ...
    encabezado = tk.Frame(ventana_principal, height=50, bg="dark violet"); encabezado.pack(side="top", fill="x")
    tk.Label(encabezado, text="Motor de Análisis de Correlación (MAC)", font=("Arial", 14, "bold"), bg="dark violet", fg="white").pack(side="left", padx=20, pady=10)
    contenedor_usuario = tk.Frame(encabezado, bg="dark violet"); contenedor_usuario.pack(side="right", padx=10)
    tk.Label(contenedor_usuario, text=usuario_actual, font=("Arial", 12), bg="dark violet", fg="white").pack(side="left", padx=10)
    tk.Button(contenedor_usuario, text="Cerrar Sesión 🚪", bg="#DC3545", fg="white", font=("Arial", 10), command=lambda: cerrar_sesion(ventana_principal)).pack(side="left", padx=10)

    menu_lateral = tk.Frame(ventana_principal, width=200, bg="white"); menu_lateral.pack(side="left", fill="y")
    tk.Frame(ventana_principal, width=1, bg="#CCCCCC").pack(side="left", fill="y")
    panel_derecho = tk.Frame(ventana_principal, bg="white"); panel_derecho.pack(side="right", expand=True, fill="both")
    ...
    def mostrar_perfil(panel_contenedor):
        ...
        tk.Label(contenedor_perfil, text="Información de Mi Perfil", font=("Arial", 16, "bold"), bg="white").pack(pady=10)
        ...
        tk.Label(formulario, text="Nombre:", bg="white", anchor="e", width=20).grid(row=0, column=0, padx=5, pady=5)
        tk.Entry(formulario, textvariable=nombre_var, width=40).grid(row=0, column=1, padx=5, pady=5)
        tk.Label(formulario, text="Apellido:", bg="white", anchor="e", width=20).grid(row=1, column=0, padx=5, pady=5)
        tk.Entry(formulario, textvariable=apellido_var, width=40).grid(row=1, column=1, padx=5, pady=5)
        tk.Label(formulario, text="Email:", bg="white", anchor="e", width=20).grid(row=2, column=0, padx=5, pady=5)
        tk.Entry(formulario, textvariable=email_var, width=40).grid(row=2, column=1, padx=5, pady=5)
        tk.Label(formulario, text="Edad:", bg="white", anchor="e", width=20).grid(row=3, column=0, padx=5, pady=5)
        tk.Entry(formulario, textvariable=edad_var, width=40).grid(row=3, column=1, padx=5, pady=5)
        tk.Label(formulario, text="Género:", bg="white", anchor="e", width=20).grid(row=4, column=0, padx=5, pady=5)
        tk.Entry(formulario, textvariable=genero_var, width=40).grid(row=4, column=1, padx=5, pady=5)
        ...
        def actualizar_info():
            ...
            try:
                ...
                if not all([nombre, apellido, email, genero]):
                    messagebox.showerror("Error de Validación", "Ningún campo puede estar vacío.", parent=panel_contenedor)
                    return
                if "@" not in email or "." not in email:
                    messagebox.showerror("Error de Validación", "El formato del email no es válido.", parent=panel_contenedor)
                    return
            except ValueError:
                messagebox.showerror("Error de Validación", "La edad debe ser un número entero válido.", parent=panel_contenedor)
                return

            USUARIOS_REGISTRADOS[usuario_actual].update({
                'nombre': nombre, 'apellido': apellido, 'email': email, 'edad': edad, 'genero': genero
            })
            guardar_usuarios()
            messagebox.showinfo("Actualización Exitosa", "La información de tu perfil ha sido guardada.", parent=panel_contenedor)

        tk.Button(contenedor_perfil, text="Actualizar Información", bg="#1976D2", fg="white",
                  font=("Arial", 12, "bold"), width=25, command=actualizar_info).pack(pady=20)
    ...
    opciones_menu_map = {
        ...
        "Análisis Estadístico": lambda: mostrar_panel_vacio(panel_derecho, "Análisis Estadístico"),
        "Visualizaciones": lambda: mostrar_panel_vacio(panel_derecho, "Visualizaciones"),
        "Simulación Dataset": lambda: mostrar_panel_vacio(panel_derecho, "Simulación Dataset"),
...
def abrir_ventana_registro(ventana_padre, entry_usuario, entry_contrasena):
    ventana_registro = tk.Toplevel(ventana_padre); ventana_registro.title("Registrar Usuario"); ventana_registro.geometry("450x450")
    ventana_registro.transient(ventana_padre); ventana_registro.grab_set()

    tk.Label(ventana_registro, text="Complete sus Datos de Registro", font=("Arial", 14, "bold"), pady=10).pack()

    canvas = tk.Canvas(ventana_registro); canvas.pack(side="left", fill="both", expand=True)
    scrollbar = ttk.Scrollbar(ventana_registro, orient="vertical", command=canvas.yview); scrollbar.pack(side="right", fill="y")
    canvas.configure(yscrollcommand=scrollbar.set)
    form_registro = tk.Frame(canvas, padx=10, pady=10); canvas.create_window((0, 0), window=form_registro, anchor="nw")
    canvas.bind('<Configure>', lambda e: canvas.configure(scrollregion = canvas.bbox("all")))

    entradas = {}
    campos_registro = ["Nombre de Usuario:", "Contraseña:", "Nombre:", "Apellido:", "Email:", "Edad:", "Género:"]
    ...
    for i, (etiqueta, clave) in enumerate(zip(campos_registro, claves)):
        tk.Label(form_registro, text=etiqueta, anchor="w", width=20).grid(row=i, column=0, padx=5, pady=5)
        if clave == "genero":
            entrada = ttk.Combobox(form_registro, textvariable=genero_var, values=["Femenino", "Masculino", "Otro"], state="readonly", width=28)
        else:
            entrada = tk.Entry(form_registro, width=30)
            if clave == "contrasena": entrada.config(show="*")
        entrada.grid(row=i, column=1, padx=5, pady=5)
        entradas[clave] = entrada

    def guardar_registro():
        datos = {clave: entradas[clave].get().strip() for clave in claves if clave != 'genero'}
        datos['genero'] = genero_var.get()

        if any(not v for k, v in datos.items() if k != 'genero') or datos['genero'] == "-- Seleccionar --":
        ...
        if datos['usuario'] in USUARIOS_REGISTRADOS:
            messagebox.showerror("Error", "El nombre de usuario ya existe.", parent=ventana_registro)
            return
        ...
        try:
            ...
            messagebox.showerror("Error", "La edad debe ser un número.", parent=ventana_registro)
            return
        ...
        messagebox.showinfo("Registro Exitoso", f"Usuario '{usuario_reg}' registrado. Ya puedes iniciar sesión.", parent=ventana_registro)
        ventana_registro.destroy()
        
        entry_usuario.delete(0, tk.END); entry_usuario.insert(0, usuario_reg)
        entry_contrasena.delete(0, tk.END); entry_contrasena.insert(0, datos["contrasena"])

    tk.Button(form_registro, text="Registrar y Volver", bg="#4CAF50", fg="white", font=("Arial", 10, "bold"), command=guardar_registro).grid(row=len(campos_registro), column=0, columnspan=2, pady=20)
...
def abrir_ventana_login():
    ...
    tk.Button(contenedor_formulario, text="Ingresar", bg="#1976D2", fg="white", width=20, command=verificar_credenciales).pack(pady=20)
